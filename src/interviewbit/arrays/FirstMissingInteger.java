...
public class FirstMissingInteger {
    ...
    public int firstMissingPositive3(List<Integer> A) {
        if (null == A || A.isEmpty()) {
            return 1;
        }
        for (int i = 0; i < A.size(); i++) {
            int num = A.get(i), position = num - 1;
            if (position >= 0 && num < A.size() && A.get(position) != num) {
                A.set(i, A.get(position));
                A.set(position, num);
                i--;
            }
        }
